...
export default function WavingCharacter({ size = "md", className = "", animated = true }: Props) {
  const { w, h } = dims(size);

  const bob = animated
    ? { animate: { y: [0, -1.5, 0] }, transition: { duration: 2.8, repeat: Infinity, ease: "easeInOut" as const } }
    : {};
      ...
      <motion.g {...bob}>
        ...
        {animated ? (
          <motion.path
            animate={{
              d: [
                "M 38 90 Q 25 70 20 50",
                "M 38 90 Q 18 62 12 42",
                "M 38 90 Q 25 70 20 50",
              ],
            }}
          ...
          <path d="M 38 90 Q 25 70 20 50" fill="none" stroke="black" strokeWidth="10" strokeLinecap="round" />
        )}
